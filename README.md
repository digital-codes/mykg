...
- **Incremental updates** — append new files to an existing session without re-running Pass 1; the schema is reused and only new files go through Pass 2. When new documents introduce entity types the current schema doesn't cover, a locked Pass 1 mode expands the vocabulary over changed files while preserving everything already in the schema
