...
- **Incremental updates** — append new files to an existing session without re-running Pass 1 schema induction; the current schema is reused and only new files go through Pass 2 extraction. When new documents introduce new concepts or properties the current schema doesn't cover, a locked Pass 1 schema induction mode expands the vocabulary over changed files while preserving everything already in the schema
