...
- **Incremental updates** — two modes for growing an existing session: *frozen schema* (reuse the current schema, extract only new files) or *grow schema* (run a locked Pass 1 over changed files to add new concepts and properties while preserving existing ones, then extract)
